...
float RotacionTotalDUPLICADA[4][4];
...
void CalcMatriz_RotacionTotalDUPLICADA(float Alfa,struct punto3d* e0,struct punto3d* e1 )
{
register	float	COS, SIN;
register	int	E0x,E0y,E0z;
register	float	N1,N2,N3; 
...
void VectorPorMatrizDUPLICADA(struct punto3d * Punto,float Matriz[4][4] )
{
    register float	x,y,z, h;
...
struct punto3d *Sup_Revolucion(int nptos,struct punto3d* ptos,int nMeridianos,struct punto3d* e0,struct punto3d* e1,char error )
{
        struct punto3d  *Matriz;
        int		i,ii, N;
        float		AlfaTotal;

    //si pasamos para definir el eje dos puntos iguales retorna NULL
    ...
    N = (nMeridianos+1) * nptos;	/* Se ha incluido un meridiano mas para enlazar la Malla de rotacion */
     //se mira si es posible reservar espacio para los N puntos que se van a generar
     if( (Matriz=(struct punto3d*)calloc( N, sizeof(struct punto3d )))==NULL )   MEMO_ERR

    //memcpy(destino, fuente, tamano)
    //matriz va a ser la matriz de puntos que nosostros le pasamos
    memcpy( Matriz, ptos, nptos*sizeof(struct punto3d) );

	for( i=1; i<=nMeridianos-1; i++){
	    memcpy( &(Matriz[nptos*i]), &(Matriz[nptos*(i-1)]), nptos*sizeof(struct punto3d) );

	    for( ii=0; ii<nptos; ii++)
		VectorPorMatrizDUPLICADA( &(Matriz[nptos*i +ii]), RotacionTotalDUPLICADA );
	}
    memcpy( &(Matriz[nptos*nMeridianos]), ptos, nptos*sizeof(struct punto3d) );
